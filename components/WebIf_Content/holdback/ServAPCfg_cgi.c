//##################################################################################################
//###  Function: Callback for Service Access-Point Configuration (cgi resource)
...
// For: ServAPCfg_set RPC, Finite State Machine processing State - Stage 2 
enum ServAPCfg_cgi_state
  {					// #XX for debugging
...
ServAPCfg_cgi(WebIF_HTTPDConnSlotData_t *conn)
  {
...
// store errors ...
// int stage2_ServAPCfg_errno = (enum http_errno) 0; //enum?

  int S2_Statex = 0; //test

  // load Stage 2 processing state (initial is 0)
  // enum state S2_State = (enum state) conn->S2_State;
  enum ServAPCfg_cgi_state S2_State = (enum ServAPCfg_cgi_state) S2_Statex;
