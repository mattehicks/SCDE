//##################################################################################################
//###  Function: Callback for System-on-Chip Hardware Configuration (cgi resource)
...
// For: SoCHWCfg_set RPC, Finite State Machine processing State - Stage 2 
enum SoCHWCfg_cgi_state
  {					// #XX for debugging
...
SoCHWCfg_cgi(WebIF_HTTPDConnSlotData_t *conn)
  {
...
// store errors ...
// int stage2_SoCHWCfg_errno = (enum http_errno) 0; //enum?

  int S2_Statex = 0; //test

  // load Stage 2 processing state (initial is 0)
  // enum state S2_State = (enum state) conn->S2_State;
  enum SoCHWCfg_cgi_state S2_State = (enum SoCHWCfg_cgi_state) S2_Statex;
