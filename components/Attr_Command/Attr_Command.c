...
const uint8_t helpTextz[] = "Usage: Attr <devspec> <attrName> [<attrVal>], to add attribute for <devspec>";	// CommandHelp, Len
const uint8_t helpDetailTextz[] = "Usagebwrebwerb: Attr <name> <type> <options>, to Define a device";	// CommandHelp, Len


providedByCommand_t Attr_ProvidedByCommand =
  {
   "attr"					// Command-Name of command -> should be same name as libfilename.so !
  ,4						// length of cmd
