...
function Poll_SwITCH() 
{
  ...
  RXxmlhttpSwITCH.onreadystatechange=function() {
	...
	if (RXxmlhttpSwITCH.readyState==4 && RXxmlhttpSwITCH.status>=200 && RXxmlhttpSwITCH.status<300) {
		...
		for (var i=0;data.STATUS.SwITCH[i];i++) {  //(var i=div.firstSwITCH; i<=div.lastSwITCH; i++) {
...
// 1. Row
					...
					td.id="NOS"+i;
					td.rowSpan="2";
					td.innerHTML=i;//data.STATUS.SwITCH[i].NO;
						var tr = document.createElement('tr');
						tr.id="aS"+i;
						tr.appendChild(td);
					...
					input.name="Name";
					input.id="NS"+i;
					input.slotname=data.STATUS.SwITCH[i].Name;
					input.slot=i;
					...
					input.value=data.STATUS.SwITCH[i].Name;
					...
					input.size="3";
					input.maxLength="3";
					input.value=data.STATUS.SwITCH[i].VAL;
					input.onblur=function(){this.hasFocus=false;};
					input.onfocus=function(){this.hasFocus=true;};
					...
					input.slot=i;
					input.type="range";
					input.min=data.STATUS.SwITCH[i].MIN;
					input.max=data.STATUS.SwITCH[i].MAX;
					input.value=data.STATUS.SwITCH[i].VAL;
					...
					input.size="3";
					input.maxLength="3";
					input.value=data.STATUS.SwITCH[i].DIM;
					input.onblur=function(){this.hasFocus=false;};
					input.onfocus=function(){this.hasFocus=true;};
...
// 2. Row
					...
					td.id="TISTS"+i;
					td.height="40";
					var dt = new Date(data.STATUS.SwITCH[i].TIST*1000);
...
//------------------------------------------------- Table data sync (if data) start -----------------------------------------------
					...
					//if (!x.hasFocus) x.innerHTML=data.STATUS.SwITCH[i].NO;
					x.innerHTML=i;//data.STATUS.SwITCH[i].NO;

					var x=document.getElementById('NS'+i);
					if (!x.hasFocus) x.value=data.STATUS.SwITCH[i].Name;

					var x=document.getElementById('VS'+i);
					if (!x.hasFocus) x.value=data.STATUS.SwITCH[i].VAL;

					var x=document.getElementById('DS'+i);
					if (!x.hasFocus) x.value=data.STATUS.SwITCH[i].DIM;

					var x=document.getElementById('RS'+i);
					if (!x.hasFocus) {
						x.min=data.STATUS.SwITCH[i].MIN;
						x.max=data.STATUS.SwITCH[i].MAX;
						x.value=data.STATUS.SwITCH[i].VAL;
					}
...
/*
				...
				if (!data.STATUS.SwITCH[i].NO)
					{
				if (data.STATUS.SwITCH[i].NO==i)
...
//				}
		...
		// prep for info
		var tist = new Date();
		var device_tist = new Date(data.STATUS.TIME*1000);

		// update info
		var Info = document.getElementById("NFOS");
		Info.innerHTML="Local Time: " + 
			tist.toLocaleTimeString('de-AT') +
			" - Data Validity: "+ device_tist.toLocaleTimeString('de-AT') +
